...
test.describe("Cart Tests (Scenarios 2, 3, 4)", () => {
  ...
  // ─── Scenario 2 (REQUIRED): Add a single product to cart - verify quantity & cart page
  test(
    "Scenario 2 - Add a single product to cart — verify quantity & cart page",
    { tag: "@required" },
    async ({ page, homePage, cartPage }) => {
      let productName = "";

      await allure.step(
        "Step 1: Wait for products to load on home page",
        async () => {
      ...
      await allure.step(
        "Step 2: Click Add to Cart on the first product",
        async () => {
          productName =
            (await page
      ...
      await allure.step(
        "Step 4: Verify cart has exactly 1 item with quantity 1",
        async () => {
          await ReportUtils.attachScreenshot(
      ...
      await allure.step(
        "Step 5: Verify cart title shows correct count",
        async () => {
  ...
  test(
    "Scenario 3 - Add the same product twice — quantity increments to 2",
    { tag: "@cart" },
    async ({ page, homePage, cartPage }) => {
      let productName = "";
      ...
      await allure.step(
        "Step 2: Click Add to Cart on the first product TWICE",
        async () => {
          productName =
            (await page
              .locator(".product-card")
      ...
      await allure.step(
        "Step 4: Verify quantity is 2 and only 1 line item exists",
        async () => {
          await ReportUtils.attachScreenshot(
            "cart with qty 2",
            page,
  ...
  test(
    "Scenario 4 - Remove items from cart",
    { tag: "@cart" },
    async ({ page, homePage, cartPage }) => {
      await allure.step(
        "Step 1: Add two different products to cart",
        async () => {
          await expect(page).toHaveURL("/home");
          await expect(async () => {
            const count = await homePage.getProductCardsCount();
