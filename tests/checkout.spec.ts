...
test.describe("Scenario 5: Checkout with valid receiver info COD", () => {
  let api: ApiHelper;
  ...
  test.beforeEach("Login and seed cart", async ({ loginPage, page }) => {
    ...
    await allure.step(
      "Step 3: Wait for home page and add a product",
      async () => {
  ...
  test(
    "S5 - Checkout with valid receiver info and COD payment",
    { tag: "@required" },
        ...
        async () => {
          ...
          await ReportUtils.attachScreenshot("order placed", page, async () => {
            ...
            const successText = page.locator(
              "text=/Order Placed Successfully/i",
            );
