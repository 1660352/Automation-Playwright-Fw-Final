...
test.describe("Scenario 1: Login fails with blank/wrong credentials", () => {
  ...
  test(
    "S1a - Login fails when submitting blank username and password",
    { tag: "@login" },
    async ({ page, loginPage }) => {
      ...
      await allure.step(
        "Step 2: Verify user stays on login page (not redirected to /home)",
        async () => {
  ...
  test(
    "S1b - Login fails with incorrect credentials",
    { tag: "@login" },
    async ({ page, loginPage }) => {
      await allure.step(
        "Step 1: Enter wrong username and password",
        async () => {
          await loginPage.doLogin("invalid_user", "invalid-pwd");
        },
      );
