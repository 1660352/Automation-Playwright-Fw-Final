...
test.describe("Login Tests - Admin Role", () => {
  var account = accounts.find((account) => account.role === "admin") || {
    username: "",
    password: "",
  };
  ...
  test(
    "ADMIN - Login successfully with valid username and password",
    { tag: "@admin" },
    async ({ page }) => {
  ...
  test(
    "ADMIN - Verify products are displayed in Featured Products section",
    { tag: "@admin" },
    async ({ page, homePage }) => {
      await allure.step(
        "Verify products are displayed in Featured Products section",
        async () => {
          await ReportUtils.attachScreenshot(
...
test.describe("Login Tests - Customer Role", () => {
  var account = accounts.find((account) => account.role === "customer") || {
    username: "",
    password: "",
  };
  ...
  test(
    "Customer - Login successfully with valid username and password",
    { tag: "@customer" },
    async ({ page }) => {
      await allure.step("Verify successful login", async () => {
        await ReportUtils.attachScreenshot(
  ...
  test(
    "Customer - User able to view users profile successfully",
    { tag: "@customer" },
    async ({ page, profilePage, homePage }) => {
      ...
      await allure.step(
        "Step 2: Verify user profile page is displayed",
        async () => {
          await ReportUtils.attachScreenshot(
      ...
      await allure.step(
        "Step 3: Verify user profile information is displayed correctly",
        async () => {
          await ReportUtils.attachScreenshot(
