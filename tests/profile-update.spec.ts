...
test.describe('Scenario 6: Update Full Name then clean up via API (REQUIRED)', () => {
    ...
    test('S6 - Update Full Name on profile page then restore via API', { tag: "@required" }, async ({ page, profilePage, homePage }) => {
        ...
        await allure.step('Step 2: Verify current name is displayed', async () => {
            await ReportUtils.attachScreenshot('profile before update', page, async () => {
        ...
        await allure.step('Step 3: Update Full Name', async () => {
            await ReportUtils.attachScreenshot('filling new name', page, async () => {
                await profilePage.updateFullName(newName);
            });
        });

        await allure.step('Step 4: Verify name persisted on page reload', async () => {
            await page.waitForTimeout(1000);
            await page.reload();
            await page.waitForLoadState('networkidle');
            await ReportUtils.attachScreenshot('profile after update', page, async () => {
                await expect(async () => {
        ...
        await allure.step('Step 6: Clean up — restore original name via API', async () => {
            const restored = await api.updateProfile(originalName);
            expect(restored.name).toBe(originalName);
        });

        await allure.step('Step 7: Verify cleanup — reload profile and confirm original name', async () => {
            await page.reload();
            await page.waitForLoadState('networkidle');
            await ReportUtils.attachScreenshot('profile after cleanup', page, async () => {
                await expect(async () => {
